...
$('#checkBasket').on('click', function() {
    ...
    for (let i = 0; i < allproducts.length; i++) {
        ...
        const productName = $productData[1].innerText.trim();
    ...
    //foreach item in local storage
    // if key=allproduxt in basket.key, localstorage.remove(key)
...
function updateTotal($target) {
    ...
    const prodPrice = $target.parent().siblings('.productPrice');
    const price = prodPrice.text();

    const quantity = $target.siblings('.quantity').val();
    //Total for one product
    let subtotal = Number(price) * Number(quantity);
    //Round to 2 decimal places.
    subtotal = subtotal.toFixed(2);
    //Display subtotal in HTML element
    const total = $target.parent().siblings('.calculateTotal');
    total.text(subtotal);
    calculateTotal();
}
...
function calculateTotal() {
    let sum = 0;
    const totalsEachProduct = $('.calculateTotal');


//not good innertext vij s jquery imasheza iteraciq
    for (let i = 0; i < totalsEachProduct.length; i++) {
        const $prod = $(totalsEachProduct[i]);
        const subTotal = Number($prod.text());
...
function updateBasketIfquantityModified() {
    ...
    for (let i = 0; i < allProductsinBasket.length; i++) {
        ...
        //productKeysInBasket=array of the keys,
        // matching each product in local storage
        productKeysInBasket.push(productKey);

        //select Dom element who cantains the atribute key
        const $prod = $('.productInBasket').filter(function() {
            return $(this).attr('data-key') === productKey;
        });
        //select quantity from the input field,which is next dom element
        const input = $prod.next().children()[2].value;


        const productInLocal = JSON.parse(localStorage[productKey]);
        ...
        // with the correct quantity and total
        else {
    ...
    for (const key of keysLocalStorage) {
        let matched = false;
        if (Number(key)) {
            for (const basketKey of productKeysInBasket) {
                if (basketKey === key) {
                    matched = true;
                }
            }
